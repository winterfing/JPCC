...
public class ClassUtil
{
    ...
    @SuppressWarnings("rawtypes")
    public static Object getNewObject(Class mClass, String className)
            throws Exception
    {
        // if(className == null || "".equals(className)){
        // className = "com.winter_ing.jpcc.handle.impl.HttpHandleImpl";
        // }
        if (getClassByClassName(className) == null)
        {
            System.out.println("not find class：" + className);
        }
        List<Class> classList = ClassUtil.getAllClassByInterface(mClass);
        if (classList == null || classList.size() == 0)
        {
            System.out.println("没有找到" + mClass.getName() + "接口的实现类："
                    + className);
        }
